...
// This series of tests still requires `service_server` and `backend_server` to
// run simultaneously
class CommandLineTest : public ::testing::Test {
 protected:
  void SetUp() override { username = "user"; }
...
TEST_F(CommandLineTest, RegisterTest) {
  ...
  std::string expected_success_output =
      std::string("Registered username: ") + username + std::string(": ") +
      command_tool::service_client.ErrorMsgs[ServiceClient::OK] +
      std::string("\n");
  ...
  // check whether the output message is correct
  std::string expected_fail_output =
      std::string("Registered username: ") + username + std::string(": ") +
      command_tool::service_client.ErrorMsgs[ServiceClient::USER_EXISTS] +
      std::string("\n");
...
TEST_F(CommandLineTest, ChirpTest) {
  ...
  // ServiceClient::ReturnCodes
  auto ret =
      command_tool::Chirp(std::string("non-existed"), std::string("text"));
  // This should fail
  EXPECT_EQ(ServiceClient::USER_NOT_FOUND, ret);
  // check whether the output message is correct
  std::string expected_fail_output =
      std::string("Posted a chirp as non-existed: ") +
      command_tool::service_client.ErrorMsgs[ServiceClient::USER_NOT_FOUND] +
      std::string("\n");
  EXPECT_EQ(expected_fail_output, buffer.str());
  // clean the `buffer
  buffer.str(std::string());
  ...
  EXPECT_EQ(ServiceClient::OK, ret);
  // check whether the output message is correct
  std::regex expected_single_output(
      std::string("Posted a chirp as ") + username + std::string(": ") +
      command_tool::service_client.ErrorMsgs[ServiceClient::OK] +
      std::string("\n\nID: [0-9]*\n@") + username + " \u00B7 .*\ntext\n");
  ...
  // check whether the output message is correct
  std::string expected_fail_reply_output =
      std::string("Posted a chirp as ") + username + std::string(": ") +
      command_tool::service_client
          .ErrorMsgs[ServiceClient::REPLY_ID_NOT_FOUND] +
      std::string("\n");
  ...
  EXPECT_EQ(ServiceClient::OK, ret);
  // check whether the output message is correct
  std::regex expected_reply_output(
      std::string("Posted a chirp as ") + username + std::string(": ") +
      command_tool::service_client.ErrorMsgs[ServiceClient::OK] +
      std::string("\n\nID: [0-9]*\n@") + username +
...
TEST_F(CommandLineTest, FollowTest) {
  ...
  // check whether the output message is correct
  std::string expected_fail_output =
      std::string("Followed ") + non_existed + std::string(" as ") + username +
      std::string(": ") +
      command_tool::service_client
          .ErrorMsgs[ServiceClient::FOLLOWEE_NOT_FOUND] +
      std::string("\n");
  EXPECT_EQ(expected_fail_output, buffer.str());
  // clean the `buffer
  buffer.str(std::string());
  ...
  std::string expected_output =
      std::string("Followed ") + username + std::string(" as ") + username +
      std::string(": ") +
      command_tool::service_client.ErrorMsgs[ServiceClient::OK] +
      std::string("\n");
...
TEST_F(CommandLineTest, ReadTest) {
  ...
  std::regex expected_output(
      std::string("Read a chirp with id [0-9]*: ") +
      command_tool::service_client.ErrorMsgs[ServiceClient::OK] + "\n" + "\n" +
      "(-*)\n" + "ID: [0-9]*\n" + "@" + username + " \u00B7 .*\n" + "text\n" +
      "(-*)\n" + ".ID: [0-9]*\n" + ".@" + username + " \u00B7 .*\n" +
      ".Reply: [0-9]*\n" + ".text\n" + "(-*)\n" + "..ID: [0-9]*\n" + "..@" +
      ...
      "..Reply: [0-9]*\n" + "..text\n" + "(-*)\n" + ".ID: [0-9]*\n" + ".@" +
      username + " \u00B7 .*\n" + ".Reply: [0-9]*\n" + ".text\n" + "(-*)\n");
  EXPECT_TRUE(std::regex_match(buffer.str(), expected_output))
      << buffer.str() << std::endl;
