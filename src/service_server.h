...
// It implements `registeruser`, `chirp`, `follow`, `read`, and `monitor` operations
class ServiceImpl final : public chirp::ServiceLayer::Service {
 ...
 private:
  // This instantiate a ServiceDataStructure so that those operations above
  // can leverage this.
  ServiceDataStructure service_data_structure_;

  // This is a helper function that helps translate a `ServiceDataStructure::Chirp`
  // object to a grpc version of `chirp::Chirp` object.
